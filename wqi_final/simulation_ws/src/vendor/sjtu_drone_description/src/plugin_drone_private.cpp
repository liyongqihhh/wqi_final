  ...
  std::string switch_mode_topic_)
{
  ...
  landing_height_subscriber_ = ros_node_->create_subscription<std_msgs::msg::Float64>(
    "landing_height", qos,
    std::bind(
      &DroneSimpleControllerPrivate::LandingHeightCallback, this,
      std::placeholders::_1));
}
...
void DroneSimpleControllerPrivate::LandCallback(const std_msgs::msg::Empty::SharedPtr msg)
{
  if (navi_state == FLYING_MODEL || navi_state == TAKINGOFF_MODEL) {
    ...
    cmd_vel.linear.x = pose.Pos().X();
    cmd_vel.linear.y = pose.Pos().Y();
    cmd_vel.linear.z = landing_height_.load();
    controllers_.roll.reset();
    controllers_.pitch.reset();
    controllers_.yaw.reset();
    controllers_.velocity_x.reset();
...
void DroneSimpleControllerPrivate::LandingHeightCallback(
  const std_msgs::msg::Float64::SharedPtr msg)
{
  if (!std::isfinite(msg->data) || msg->data < 0.0 || msg->data > 2.0) {
    RCLCPP_ERROR(
      ros_node_->get_logger(), "Rejected landing height %.3f m", msg->data);
    return;
  }
  landing_height_.store(msg->data);
  RCLCPP_INFO(
    ros_node_->get_logger(), "Landing height set to %.3f m", msg->data);
}
  ...
  const ignition::math::v6::Vector3<double> & acceleration)
{
  ...
  std::string frame_namespace = ros_node_->get_namespace();
  while (!frame_namespace.empty() && frame_namespace.front() == '/') {
    frame_namespace.erase(0, 1);
  }
  const std::string frame_prefix =
    frame_namespace.empty() ? "" : frame_namespace + "/";
  odom.header.frame_id = frame_prefix + "odom";
  odom.child_frame_id = frame_prefix + "base_footprint";
 ...
 * Takeoff enters the flying state after the initial thrust phase. Landing stays
 * active until the physical vehicle reaches the configured ground height.
*
...
void DroneSimpleControllerPrivate::UpdateState(double dt)
{
  if (navi_state == TAKINGOFF_MODEL) {
    ...
    m_timeAfterCmd += dt;
    const bool reached_ground =
      pose.Pos().Z() <= landing_height_.load() + landing_tolerance_;
    if (m_timeAfterCmd > 0.2 && reached_ground) {
      link->SetLinearVel(ignition::math::v6::Vector3<double>(0.0, 0.0, 0.0));
      link->SetAngularVel(ignition::math::v6::Vector3<double>(0.0, 0.0, 0.0));
      navi_state = LANDED_MODEL;
...
void DroneSimpleControllerPrivate::UpdateDynamics(double dt)
{
  ...
  if (m_posCtrl) {
    //position control
    if (navi_state == FLYING_MODEL || navi_state == LANDING_MODEL) {
      ...
      double vz = navi_state == LANDING_MODEL ?
        -landing_velocity_ :
        controllers_.pos_z.update(cmd_vel.linear.z, position[2], poschange[2], dt);
    ...
    const double vertical_velocity_command =
      navi_state == LANDING_MODEL ? -landing_velocity_ : cmd_vel.linear.z;
    force[2] = mass *
      (controllers_.velocity_z.update(
        vertical_velocity_command, velocity[2], acceleration[2],
        dt) + load_factor * gravity);
  }
