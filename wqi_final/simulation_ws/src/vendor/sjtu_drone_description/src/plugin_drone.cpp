...
void DroneSimpleController::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("bodyName")) {
    ...
    // Link names are not globally unique in a multi-robot world. Resolve the
    // controlled body inside this plugin's model so another robot cannot
    // receive the UAV forces and odometry.
  ...
  if (_sdf->HasElement("landingVelocity")) {
    impl_->landing_velocity_ = _sdf->GetElement("landingVelocity")->Get<double>();
  }
  if (_sdf->HasElement("landingHeight")) {
    impl_->landing_height_ = _sdf->GetElement("landingHeight")->Get<double>();
  }
  if (_sdf->HasElement("landingTolerance")) {
    impl_->landing_tolerance_ = _sdf->GetElement("landingTolerance")->Get<double>();
  }
  ...
  RCLCPP_INFO_STREAM(
    impl_->ros_node_->get_logger(), "Using following parameters: \n" <<
      ...
      "\t\tlanding_velocity: " << impl_->landing_velocity_ << ",\n" <<
      "\t\tlanding_height: " << impl_->landing_height_.load() << ",\n" <<
      "\t\tlanding_tolerance: " << impl_->landing_tolerance_
  );
